...
exports.acknowledgeViolation = acknowledgeViolation;
exports.exportViolationsCSV = exportViolationsCSV;
...
async function acknowledgeViolation(req, res) {
    try {
        ...
        const violation = await db_1.prisma.violation.findUnique({
            where: { id },
            include: {
                worker: true,
            },
        ...
        // Update violation
        const updatedViolation = await db_1.prisma.violation.update({
            where: { id },
            data: {
                isAcknowledged: true,
        ...
        simulator_1.simulatorService.clearEscalatedId(id);
        // Broadcast acknowledgement to all connected sockets
        websocket_1.webSocketService.broadcast('VIOLATION_ACKNOWLEDGED', updatedViolation, violation.worker.siteId);
...
async function exportViolationsCSV(req, res) {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ error: 'Unauthorized' });
